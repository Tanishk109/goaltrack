...
router.get('/schedule', authenticate, async (req, res) => {
  try {
    ...
    const schedule = getScheduleForCycle(cycle);
    const periods = await CheckInPeriod.find({ cycle: cycleId }).populate('launchedBy', 'name role');
    const periodMap = {};
    periods.forEach((p) => { periodMap[p.phase] = p; });
    ...
    const phases = await Promise.all(schedule.map(async (s) => {
      ...
      if (now >= s.windowOpens) calendarStatus = 'open';
      if (s.windowCloses && now > s.windowCloses) calendarStatus = 'past';
...
router.post('/launch', authenticate, authorize('manager', 'admin'), async (req, res) => {
  try {
    ...
    const deadlineDate = deadline ? new Date(deadline) : defaultDeadline(windowOpens, phase);
